...
const SingleVendorPage = () => {
  ...
  const {customer_items_loading, customer_vendor_items} = useSelector(state => state.menuItem)
  const [item_qtr, setQuantity] = useState(0)
  const {id} = useParams()
...
const snacks = customer_vendor_items.filter(item => item.item_category === 'Snacks')
const food =  customer_vendor_items.filter(item => item.item_category === 'Food')
const drinks =  customer_vendor_items.filter(item => item.item_category === 'Drinks')
...
// const [item, setItems] = useState(menuItems)

const handleQtyChange = (id, quantity) =>{
  const item = customer_vendor_items.filter(item => item.id === id)
   if (item.id = id){setQuantity(quantity)}
}
  ...
  return (
    ...
    <section id='hero-section' className='flex justify-between w-[90%] ml-auto mr-auto  px-10 gap-28 bg-slate-200 rounded-xl h-56'>
      ...
      <div className='w-1/2 mt-10'>
        <p className='text-lg font-base text-gray-600 w-[80%] h-20 overflow-hidden'>
          {customer_vendor_profile.vendor_description}
        </p>

        <div className='flex gap-4 mt-11'>
          <span className='inline-flex items-center gap-2 text-xs text-gray-600'><AiOutlineClockCircle/> <p>Open: {customer_vendor_profile.vendor_openining_hours}</p></span>
          <span className='inline-flex items-center gap-2 text-xs text-gray-600'><CiLocationOn/> <p>{customer_vendor_profile.vendor_address}</p></span>
          <span className='inline-flex items-center gap-2 text-xs text-gray-600'><BsTelephone/> <p>{customer_vendor_profile.vendor_contact_number}</p></span>
        </div>
      </div>
      <div className='w-1/2 mt-20'>
        <img src={customer_vendor_profile?.vendor_logo?.startsWith('http') 
    ? customer_vendor_profile.vendor_logo 
    : `http://localhost:8000${customer_vendor_profile.vendor_logo}`  }  className='rounded-xl h-52 w-[70%] shadow-md'/>
      </div>
    </section>
    ...
    {customer_items_loading ?  (<div className='w-[90%] mr-auto ml-auto mt-10 flex justify-center items-center'><h1>loading...</h1></div>): customer_vendor_items.length === 0? (<div className='w-[90%] mr-auto ml-auto mt-10 flex justify-center items-center'><h1>NO Items</h1></div>): (<>
      ...
      <div className='flex flex-wrap justify-center items-center gap-5 mt-5'>
        {foodToDisplay.map((item) => ( <div key={item.id} className='flex flex-col w-72 h-80 bg-slate-200 shadow-sm hover:shadow-2xl border-2 border-gray-300'>
          <div className='h-[57%] w-full'>
            <img src={`http://localhost:8000${item.item_photo}`}  className='h-full w-full'/>
          </div>
          
          <div className='flex flex-col px-2 pb-4'>
            <div className='flex justify-between'>
              <h2 className='font-base text-sm text-gray-900 font-semibold mt-2'>{item.item_name}</h2>
            ...
            <div className='flex  h-12'>
              <p className='font-base text-sm  mt-2 text-gray-600 t'>{item.item_description}</p>
            </div> 
            <div className='flex justify-between items-center mt-1'>
              ...
              onClick={() => addToCart(item.id, item_qtr )}
              />
            </div>
          </div>
        </div>
